...
try:
    ...
    global_threshold = 0.000251 # Hata durumunda yedek değer
...
# Sidebar Mod Seçimi
mod = st.sidebar.radio("Eşik Değer Belirleme Modu", ["Otomatik (Global Eşik)", "Manuel (Özel Değer)"])

if mod == "Otomatik (Global Eşik)":
    esik_deger = global_threshold
    st.sidebar.info(f"Otonom Eşik: {esik_deger:.6f}")
else:
    esik_deger = st.sidebar.slider("Manuel Eşik Değeri", 0.0001, 0.0100, value=global_threshold, step=0.0001, format="%.6f")
...
# 📊 STATİK ANALİZ (Dosya seçildiği an hesaplanır)
if secilen_dosya:
    ...
    with st.spinner("Statik analiz grafiği hazırlanıyor..."):
        ...
        # GRAFİK ÇİZİMİ - Değişiklik burada!
        fig, ax = plt.subplots(figsize=(10, 3))
        ax.plot(mse_tam, color='#2ca02c', alpha=0.6, label='MSE Loss')
        ax.axhline(y=esik_deger, color='red', linestyle='--', label='Eşik Değer')
        ax.set_title("Uçuş Boyunca Anomali Skoru")
        ax.legend()
        
        # st.pyplot'u bir container içinde çağırıyoruz, bu genelde hatayı çözer
        st.pyplot(fig)
        
        # Belleği temizle (matplotlib grafiklerini kapat)
        plt.close(fig)
...
if start_btn and secilen_dosya:
    ...
    for idx in range(10, len(df_flight)):
        ...
        chart_df = pd.DataFrame({
            "Anlık Hata": loss_history[-grafik_penceresi:],
            "Eşik": [esik_deger] * len(loss_history[-grafik_penceresi:])
        }, index=step_history[-grafik_penceresi:])
        
        chart_area.line_chart(chart_df, height=500, color=["#1f77b4", "#ff4b4b"])
